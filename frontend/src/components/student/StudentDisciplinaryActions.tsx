...
const StudentDisciplinaryActions: React.FC = () => {
  ...
  useEffect(() => {
    const loadDisciplinaryActions = async () => {
      try {
        setLoading(true);
        const response = await studentApi.getDisciplinaryActions();
        ...
        console.error('Failed to load red warrants:', error);
        toast.error('Failed to load red warrants');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">My Red Warrants</h2>
          <p className="text-gray-600">View your red warrants and current status</p>
        </div>
      </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2 text-xs">
                  ...
                  <span className={`px-2 py-1 rounded ${action.studentAcknowledged ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                    {action.studentAcknowledged ? 'Acknowledged' : 'Not Acknowledged'}
                  </span>
      ...
      {actions.length === 0 && (
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-gray-600">You have no red warrants! Keep up the good behavior.</p>
          </CardContent>
        </Card>
      )}
    </div>
