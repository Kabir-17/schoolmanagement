...
const SuperadminHome: React.FC<SuperadminHomeProps> = ({
  ...
  // Add some mock pending schools for demonstration if no real pending schools exist
  ...
  // Use real schools if available, otherwise include mock data for demonstration
  const hasPendingSchools = safeSchools.some(school => school.status === 'pending_approval');
  const schoolsWithMockData = hasPendingSchools ? safeSchools : [...safeSchools, ...mockPendingSchools];
  const statCards = [
    {
      title: "Total Schools",
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-6">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {statCards.map((stat) => (
            <Card key={stat.title} className="bg-white/80 backdrop-blur-sm border-0 shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center">
              ...
              Pending Schools
              {(() => {
                const pendingCount = schoolsWithMockData.filter(school => school.status === 'pending_approval').length;
          ...
          <CardContent>
            {(() => {
              const pendingSchools = schoolsWithMockData.filter(school => school.status === 'pending_approval');
              return pendingSchools.length > 0 ? (
                ...
                <div className="text-center py-8">
                  {loading ? (
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-green-600 font-medium mb-2">All caught up!</p>
                      <p className="text-xs text-gray-400">No schools pending approval at the moment.</p>
                    </div>
                  )}
                </div>
                );
        ...
        </Card>        {/* System Health with Quick Actions */}
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center">
